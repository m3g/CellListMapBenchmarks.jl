...
@with_kw struct Params{V,N,T,M,UnitCellType}
    ...
    # Parameters for Neon
    mass::Vector{T} = T[ 1.00800, 1.00784, 15.99940 ] # g/mol 
    charge::Vector{T} = T[-0.834, 0.417, 0.417]
    ε::Vector{T} = T[-0.1521, -0.046, -0.046] # kcal/mol
    σ::Vector{T} = T[1.7682, 0.2245, 0.2245] # Å
    bonds_kb::Vector{T} = T[ 450., 0. ] # kcal/mol/Å^2
    bonds_b0::Vector{T} = T[ 1.539, 0.9572, 0.9572 ] # Å
    angle::Vector{T} = T[55.0, 104.52] # kcal/mol/rad^2, degrees
    kB::T = 0.001985875 # Boltzmann constant kcal / mol K
end

function nonbonded_potential(i,j,d2,ε,σ,u,mass)
    @fastpow u += ε*( σ^12/d2^6 - 2*σ^6/d2^3 ) + (1/(4πε₀))*mass[i]*mass[j]/sqrt(d)
    return u
end

function nonbonded_forces!(x,y,i,j,d2,ε,σ,f,charge)
    r = y - x
    @fastpow dudr = (-12*ε*(σ^12/d2^7 - σ^6/d2^4) - 1/(4πε₀)*charge[i]*charge[j]/dr3)*r
    @inbounds f[i] = f[i] + dudr
    @inbounds f[j] = f[j] - dudr
    return f
end

function bonded_forces!(f,x,mass,bonds_kb,bonds_b0,angle)
    for iatom in 1:3:natoms
        iO = iatom
        iH1 = iatom + 1
        iH2 = iatom + 2
        rOH1 = norm(x[iO] - x[iH1])
        rOH2 = norm(x[iO] - x[iH2])
        rH1H2 = norm(x[iH1] - x[iH2])
        f_bond_OH1 = -bonds_k0[iO]*rOH1^2
        f_bond_OH2 = -bonds_k0[iO]*rOH2
        f_bond_H1H2 = -bonds_k0[iH1]*rH1H2
        f[iO] = 
